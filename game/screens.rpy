...
screen say(who, what):
    ...
    if renpy.variant("small"):
        add Transform(SideImage(), xoffset=10, zoom=0.9) xalign 0.0 yalign 1.0
    else:
        add Transform(SideImage(), xoffset=50) xalign 0.0 yalign 1.0
...
style nvl_window:
    xfill True
    xmaximum 1000       # ограничиваем ширину NVL окна
    xpos 0.5
    xanchor 0.5         # жёстко центрируем по горизонтали
    yfill True
    ymaximum 600        # ограничиваем высоту NVL окна
    ypos 0.4
    yanchor 0.5         # жёстко центрируем по вертикали
